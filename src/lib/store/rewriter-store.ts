...
const initialState: RewriterState = {
  ...
  // Rewrite Options
  preserveExistingDesignTools: true,
  skipAlreadyStyled: false,
  activePatterns: [],

  // DesignTools Formatting Options
  designToolsOptions: {
    theme: "default",
    ...
    addCallouts: false,
    calloutDetection: "auto" as const,
    frameImages: false,
...
export const useRewriterStore = create<RewriterStore>()(
  persist(
    (set, get) => ({
      ...
      // Rewrite Options
      setPreserveExistingDesignTools: (preserve) =>
        set({ preserveExistingDesignTools: preserve }),
      setSkipAlreadyStyled: (skip) => set({ skipAlreadyStyled: skip }),
      setActivePatterns: (patterns) => set({ activePatterns: patterns }),
      togglePattern: (patternId) =>
        set((state) => ({
          activePatterns: state.activePatterns.includes(patternId)
            ? state.activePatterns.filter((p) => p !== patternId)
            : [...state.activePatterns, patternId],
        })),

      // DesignTools Formatting Options
      setDesignToolsOptions: (options) =>
        set((state) => ({
          designToolsOptions: { ...state.designToolsOptions, ...options },
        })),
